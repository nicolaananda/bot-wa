...
const BOOKING_HISTORY_MS = 30 * 24 * 60 * 60 * 1000

// Tier yang meeting-nya dibuat sebagai "Previous" (backdated).
// Bisa di-override via env ZOOM_BACKDATE_TIERS="100,300,500,1000".
function backdateTiers() {
  const raw = String(process.env.ZOOM_BACKDATE_TIERS || '100,300,500,1000').trim()
  return new Set(
    raw
      .split(',')
...
// peduli durasi asli yang dibeli customer. Default 60 (1 jam).
function backdateZoomDurationMinutes() {
  const n = Number(process.env.ZOOM_BACKDATE_ZOOM_DURATION_MINUTES)
  return Number.isFinite(n) && n > 0 ? Math.floor(n) : 60
}

// Seberapa jauh END meeting diletakkan relatif "sekarang" (menit).
// Kalau positif: end di masa lalu (Previous). Kalau negatif: end di masa depan.
// Kamu mau end = now + 1 menit (masih Upcoming 1 menit), jadi lead = -1.
 ...
 *   => end   = start + duration = sekarang - endLead
 *
 * Contoh (sekarang 14:25, default):
 *   duration = 60, endLead = -1
 *   start = 14:25 - (60 + (-1)) = 14:25 - 59 = 13:26 kemarin
 *   end = 13:26 + 60 = 14:26 → 1 menit ke depan (masih Upcoming)
 *
...
/**
 * Simpan histori 30 hari agar meeting yang sudah selesai masih dapat dipetakan
 * ke akun host saat cloud recording diminta.
 */
function cleanupExpired(store) {
  const cutoff = Date.now() - BOOKING_HISTORY_MS
  for (let i = store.length - 1; i >= 0; i--) {
    const b = store[i]
    const timestamp = Number(b && (b.realEndUtcMs || b.createdAt))
    if (!b || !Number.isFinite(timestamp) || timestamp <= cutoff) {
      store.splice(i, 1)
    }
  }
}
...
function findBookingByMeetingId(meetingId) {
  const store = bookingStore()
  if (!store) return null
  cleanupExpired(store)
  const normalizedId = String(meetingId || '').replace(/\s+/g, '')
  return (
    store.find((booking) => String((booking && booking.meetingId) || '') === normalizedId) || null
  )
}
...
function listLocalBookings(tier) {
  ...
  const now = Date.now()
  return store
    .filter((b) => b && Number(b.tier) === tierNum && Number(b.realEndUtcMs) > now)
    .map((b) => ({
...
/**
 * Hitung booking pada tanggal kalender yang sama di timezone customer.
 * Booking selesai tetap dihitung agar distribusi harian tidak menumpuk ke host
 * pertama hanya karena meeting sebelumnya sudah selesai.
 */
function countBookingsForDay({ tier, hostAccountId, startAtUtcMs, timezone }) {
  const store = bookingStore()
  if (!store) return 0
  cleanupExpired(store)
  const tz = timezone || process.env.ZOOM_DEFAULT_TIMEZONE || 'Asia/Jakarta'
  const day = moment.tz(Number(startAtUtcMs), tz).format('YYYY-MM-DD')
  return store.filter((booking) => {
    if (!booking || Number(booking.tier) !== Number(tier)) return false
    if (String(booking.hostAccountId) !== String(hostAccountId)) return false
    return moment.tz(Number(booking.realStartUtcMs), tz).format('YYYY-MM-DD') === day
  }).length
}
