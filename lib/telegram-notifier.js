...
async function sendOperationalAlert(message) {
    ...
    if (!bot || !CHAT_ID) {
        console.error(`[OPS ALERT] ${safeMessage}`);
        return false;
    }
    try {
        await bot.sendMessage(CHAT_ID, `🚨 *OPERATIONAL ALERT*\n${safeMessage}`, { parse_mode: 'Markdown' });
        return true;
    } catch (error) {
        console.error(`[OPS ALERT] ${safeMessage} (Telegram delivery failed: ${error.message})`);
        return false;
    }
}
