 ...
 *
 * Docs: GET /users/{userId}
 * @param {object} [opts]
 * @param {string} [opts.userId]
 * @returns {Promise<object>} user object (includes host_key, email, first_name, etc.)
 */
async function getUser({ userId } = {}) {
    const host = userId || process.env.ZOOM_USER_ID || 'me'
    return zoomRequest('GET', `/users/${encodeURIComponent(host)}`)
}
 ...
 * @param {boolean} [opts.notifyHosts=true]
 */
