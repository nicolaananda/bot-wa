...
async function createMeeting({
  ...
  if (password && !/^[A-Za-z0-9@_*-]{1,10}$/.test(String(password))) {
    throw new Error('password must be at most 10 characters using letters, numbers, or @ - _ *')
  }
...
/**
 * Fetch Zoom user settings — used to read meeting capacity / large meeting addon.
 * Docs: GET /users/{userId}/settings
 * Returns at minimum: { feature: { meeting_capacity, large_meeting, large_meeting_capacity, ... } }
 * Requires scope: user:read:settings:admin (or user:read:admin)
 */
async function getUserSettings({ userId, creds } = {}) {
  const resolved = resolveCreds(creds)
  const host = userId || resolved.userId
  return zoomRequest('GET', `/users/${encodeURIComponent(host)}/settings`, { creds })
}

/**
 * List users belonging to the one account represented by `creds`.
 * Requires user:read:admin (or the equivalent granular scope).
 */
async function listUsers({ status, pageSize = 100, creds } = {}) {
  const users = []
  let nextPageToken = ''

  for (let i = 0; i < 10; i++) {
    const data = await zoomRequest('GET', '/users', {
      params: {
        page_size: Math.min(Math.max(Number(pageSize) || 100, 1), 300),
        ...(status ? { status } : {}),
        ...(nextPageToken ? { next_page_token: nextPageToken } : {}),
      },
      creds,
    })
    if (Array.isArray(data && data.users)) users.push(...data.users)
    nextPageToken = data && data.next_page_token
    if (!nextPageToken) break
  }
...
/**
 * Invite a user to this account as Basic (type 1).
 * Requires user:write:admin (or the equivalent granular scope).
 */
async function createBasicUser({ email, displayName, creds } = {}) {
  ...
  return zoomRequest('POST', '/users', {
    data: {
      ...
      user_info: {
        ...
        type: 1,
        ...(displayName ? { display_name: String(displayName).slice(0, 128) } : {}),
      },
    },
    creds,
  })
...
/**
 * Disassociate a user from this account without permanently deleting it.
 */
async function disassociateUser({ userId, creds } = {}) {
  if (!userId || !String(userId).trim()) throw new Error('userId is required')
  return zoomRequest('DELETE', `/users/${encodeURIComponent(String(userId).trim())}`, {
...
/**
 * Fetch cloud recording files for one meeting.
 * Docs: GET /meetings/{meetingId}/recordings
 */
async function getMeetingRecordings({ meetingId, creds } = {}) {
  const normalizedId = String(meetingId || '').replace(/\s+/g, '')
  if (!/^\d{9,11}$/.test(normalizedId)) throw new Error('meetingId must contain 9-11 digits')
  return zoomRequest('GET', `/meetings/${encodeURIComponent(normalizedId)}/recordings`, { creds })
}
