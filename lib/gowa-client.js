...
class GowaClient {
    ...
    async deleteMessage(jid, messageId, options = {}) {
        ...
        try {
            // Try /message/revoke first (common endpoint)
            const response = await this.axios.post('/message/revoke', {
                phone: phone,
                message_id: messageId
            });
            console.log(`[GOWA-CLIENT] Revoke response: ${JSON.stringify(response.data)}`);
            return response.data;
        } catch (error) {
            // Fallback: don't throw, just log warning
            console.warn(`[GOWA-CLIENT] Warning: Failed to delete message ${messageId} for ${phone}. API might not support it. (${error.message})`);
    ...
    async groupMetadata(groupId) {
        ...
        try {
            const response = await this.axios.get(`/group/${encodeURIComponent(groupJid)}`);
