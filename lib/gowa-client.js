...
class GowaClient {
  constructor(config = {}) {
    ...
    this.axios.interceptors.response.use(
      (response) => response,
      (error) => {
        const isOptionalAvatarRequest =
          error.config?.method === 'get' && /^\/user\/[^/]+\/avatar$/.test(error.config?.url)
        if (!isOptionalAvatarRequest) {
  ...
  async sendText(jid, text, options = {}) {
    ...
    // Support alternative field names for mentions
    if (payload.mentions && !payload.mentioned_jid) {
      payload.mentioned_jid = payload.mentions
    }
  ...
  async deleteMessage(jid, messageId, options = {}) {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `[GOWA-CLIENT] Warning: Failed to delete/revoke message ${messageId} for ${jid}. (${error.message})`
        )
  ...
  async groupUpdateSubject(groupId, subject) {
    const groupJid = this._ensureGroupJid(groupId)
    const response = await this.axios.post('/group/name', {
      group_id: groupJid,
      name: subject,
    })
    return response.data
  ...
  async profilePictureUrl(jid, type = 'image') {
    try {
      const response = await this.axios.get('/user/avatar', {
        params: { phone: jid, is_preview: type !== 'image' },
      })
      const data = response.data.results || response.data
      return data.url || data.avatar_url || data.avatar
    } catch (error) {
  ...
  async groupLeave(groupId) {
    ...
    const response = await this.axios.post('/group/leave', { group_id: groupJid })
    return response.data
  }
  ...
  async groupSettingUpdate(groupId, setting, value) {
    ...
    const announce = setting === 'announcement' || setting === 'not_announcement'
    const endpoint = announce ? '/group/announce' : '/group/locked'
    const field = announce ? 'announce' : 'locked'
    const enabled =
      value === undefined ? setting === 'announcement' || setting === 'locked' : Boolean(value)
    const response = await this.axios.post(endpoint, { group_id: groupJid, [field]: enabled })
  ...
  _formatGroupMetadata(data) {
    ...
    return {
      ...
      participants: (groupData.participants || groupData.Participants || []).map((p) => ({
        id: p.PhoneNumber || p.jid || p.id || p.JID, // Prioritize PhoneNumber (@s.whatsapp.net) over LID
        lid: p.JID || p.lid, // Store LID separately if needed
        admin:
          p.is_admin || p.IsAdmin
            ? 'admin'
            : p.is_super_admin || p.IsSuperAdmin
              ? 'superadmin'
              : null,
