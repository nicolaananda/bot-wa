 ...
 *     hostKey?, concurrentMeetings?, exp?, notes? }
 *
...
const inFlightHostAccounts = new Set()
...
function getExpTimestamp(value) {
  const normalized = normalizeExp(value)
  if (!normalized) return null
  const [dd, mm, yyyy] = normalized.split('/')
  return Date.UTC(Number(yyyy), Number(mm) - 1, Number(dd), 23, 59, 59, 999)
}

function getHostExpiryStatus(host, startAtUtcMs = Date.now()) {
  const exp = normalizeExp(host && host.exp)
  if (!exp) return { ok: true, exp: null, expiresAtUtcMs: null }

  const expiresAtUtcMs = getExpTimestamp(exp)
  if (!expiresAtUtcMs) {
    return {
      ok: false,
      exp,
      status: 'expired',
      reason: 'ACCOUNT_EXP_INVALID',
      detail: `EXP host tidak valid: ${exp}`,
      expiresAtUtcMs: null,
    }
  }

  if (Number(startAtUtcMs) > expiresAtUtcMs) {
    return {
      ok: false,
      exp,
      status: 'expired',
      reason: 'ACCOUNT_EXPIRED',
      detail: `EXP host ${exp} sudah lewat untuk jadwal meeting ini.`,
      expiresAtUtcMs,
    }
  }

  return { ok: true, exp, expiresAtUtcMs }
}

function buildExpiredHostReason(host, expiryStatus) {
  return {
    label: host.label,
    status: expiryStatus.status || 'expired',
    reason: expiryStatus.reason || 'ACCOUNT_EXPIRED',
    detail: expiryStatus.detail,
    exp: expiryStatus.exp || null,
    expiresAtUtcMs: expiryStatus.expiresAtUtcMs || null,
  }
}
...
function sanitize(entry, index, tier) {
  ...
  const exp = normalizeExp(entry.exp != null ? entry.exp : entry.EXP)
...
// Larger pools are fallback stock for smaller orders. Deduplicate account IDs
// so an account added to multiple pools is still one physical Zoom host.
function loadHostsForTier(tier = 100) {
  const tierKey = Number(tier)
  ...
  const seen = new Set()
  const hosts = []
  for (const capacityTier of VALID_TIERS.filter((candidate) => candidate >= tierKey)) {
    for (const host of loadPool(capacityTier)) {
      if (seen.has(host.accountId)) continue
      seen.add(host.accountId)
      hosts.push({ host, capacityTier })
    }
  }
  return hosts
}
 ...
 */
async function _evaluateLicenseGate(host, tier, poolTier = tier) {
  ...
  if (host.disabledAt) {
    try {
      ...
      if (recheck.ok) {
        enableHost(poolTier, { accountId: host.accountId })
...
async function findFirstLicensedHost(tier = 100) {
  const candidates = loadHostsForTier(tier)
  if (!candidates.length) return { ok: false, error: 'POOL_EMPTY', reasons: [] }

  const reasons = []
  for (const { host, capacityTier } of candidates) {
    const expiryStatus = getHostExpiryStatus(host)
    if (!expiryStatus.ok) {
      reasons.push(buildExpiredHostReason(host, expiryStatus))
      continue
    }
    try {
      const gate = await _evaluateLicenseGate(host, tier, capacityTier)
      if (gate.healthy) {
        return {
          ok: true,
          poolTier: capacityTier,
          host,
          license: {
...
async function createMeetingOnFirstAvailable({
  ...
  const reasons = []
  const orderedPool = zoomBackdate.isBackdateTier(tier)
    ? candidates
        .map(({ host, capacityTier }, index) => ({
          host,
          capacityTier,
          index,
          dailyBookings: zoomBackdate.countBookingsForDay({
            tier,
        ...
        .sort(
          (a, b) =>
            a.capacityTier - b.capacityTier ||
            a.dailyBookings - b.dailyBookings ||
            a.index - b.index
        )
        .map(({ host, capacityTier }) => ({ host, capacityTier }))
    : candidates

  for (const { host, capacityTier } of orderedPool) {
    const hostReason = { label: host.label, status: 'unknown', detail: null }
    if (inFlightHostAccounts.has(host.accountId)) {
      reasons.push({ ...hostReason, status: 'busy', detail: [] })
      continue
    }
    const expiryStatus = getHostExpiryStatus(host, startAtUtcMs)
    if (!expiryStatus.ok) {
      reasons.push(buildExpiredHostReason(host, expiryStatus))
      continue
    }
    inFlightHostAccounts.add(host.accountId)
    try {
      ...
      // Catat akun pembuat untuk conflict check dan lookup cloud recording.
      ...
      inFlightHostAccounts.delete(host.accountId)
    }
  }

  const failedReasons = reasons
    .filter((reason) => reason.status === 'error' && reason.detail)
    .map((reason) => `${reason.label}: ${reason.detail}`)
  return {
    ok: false,
    reasons,
    error: 'ALL_BUSY_OR_FAILED',
    detail: failedReasons.length ? failedReasons.join(' | ') : undefined,
  }
}
...
async function findFirstAvailableHost({ tier = 100, durationMinutes, startAtUtcMs }) {
  const candidates = loadHostsForTier(tier)
  if (!candidates.length) return { ok: false, reasons: [], error: 'POOL_EMPTY' }
  if (!startAtUtcMs) throw new Error('startAtUtcMs is required')
  if (!durationMinutes || durationMinutes <= 0) throw new Error('durationMinutes must be > 0')
 ...
 */
async function _createMeetingOnHost({
  host,
  topic,
  startTimeIso,
  durationMinutes,
  ...
  if (!expiryStatus.ok) {
    ...
    return {
      ok: false,
      error: 'EARMARKED_HOST_EXPIRED',
      reasons: [buildExpiredHostReason(host, expiryStatus)],
    }
  }

  // Re-validate license at payment-confirmation time. Cheap due to cache.
  if (tier) {
    const gate = await _evaluateLicenseGate(host, tier, poolTier || tier)
    if (!gate.healthy) {
      if (allowFallback) {
        return createMeetingOnFirstAvailable({
          tier,
...
async function createMeetingOnHost(opts) {
  const accountId = opts && opts.host && opts.host.accountId
  if (accountId && inFlightHostAccounts.has(accountId)) {
    if (opts.allowFallback && opts.tier) {
      return createMeetingOnFirstAvailable({
        tier: opts.tier,
        topic: opts.topic,
        ...
        startAtUtcMs: opts.startAtUtcMs,
        agenda: opts.agenda,
      })
    }
    return {
      ok: false,
      error: 'EARMARKED_HOST_BUSY',
      reasons: [{ label: opts.host.label, status: 'busy', detail: [] }],
    }
  }

  if (accountId) inFlightHostAccounts.add(accountId)
  try {
    return await _createMeetingOnHost(opts)
  } finally {
    if (accountId) inFlightHostAccounts.delete(accountId)
  }
}
...
async function listAllUpcoming(tier = 100) {
  const pool = loadHostsForTier(tier).map(({ host }) => host)
  const result = []
  const errors = []
