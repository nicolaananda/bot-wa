...
class GowaAdapter extends EventEmitter {
    ...
    async sendMessage(jid, content, options = {}) {
        try {
            ...
            if (content.text) {
                // Ensure mentions are passed
                const msgOptions = { ...options };
                if (content.mentions) msgOptions.mentions = content.mentions;
                response = await this.client.sendText(jid, content.text, msgOptions);
    ...
    /**
     * Delete message (explicit method)
     */
    async deleteMessage(jid, messageId, options = {}) {
        const id = messageId.id || messageId.key?.id || messageId;
        return await this.client.deleteMessage(jid, id, options);
    }
    ...
    handleWebhook(webhookData) {
        try {
            ...
            if (webhookData.event && webhookData.payload) {
                // Only process 'message' events for chat responses
            ...
            if (messageId) {
                ...
                if (this.processedMessages.size > 1000) {
                    ...
                    for (const [id, ts] of this.processedMessages) {
                        if (ts < fiveMinAgo) this.processedMessages.delete(id);
                    }
                }
            }
    ...
    _convertToBaileysFormat(webhookData) {
        try {
            ...
            return {
                ...
                messageTimestamp: (() => {
                    const t = messageContent.timestamp || messageContent.time;
                    if (!t) return Math.floor(Date.now() / 1000);
                    return Number(t) > 1e12 ? Math.floor(Number(t) / 1000) : Number(t);
                })(),
    ...
    _buildMessageContent(message) {
        const content = {};
        const nestedMessage = message && typeof message.message === 'object' ? message.message : null;
        const textValue =
            (message && (message.text || message.body || message.conversation)) ||
            (message && message.extendedTextMessage && message.extendedTextMessage.text) ||
            (nestedMessage && (nestedMessage.text || nestedMessage.body || nestedMessage.conversation)) ||
            (nestedMessage && nestedMessage.extendedTextMessage && nestedMessage.extendedTextMessage.text) ||
            '';

        if (textValue) {
            content.conversation = textValue;
        } else if (message.extendedTextMessage) {
            content.extendedTextMessage = message.extendedTextMessage;
            ...
            return this._buildMessageContent(nestedMessage);
        }
