...
class GowaAdapter extends EventEmitter {
    ...
    _convertToBaileysFormat(webhookData) {
        try {
            // Gowa webhook structure varies, adapt as needed
            const message = webhookData.message || webhookData;

            return {
                key: {
                    remoteJid: message.from || message.chat || message.jid,
                    fromMe: message.fromMe || false,
                    id: message.id || message.messageId || this._generateMessageId(),
                    participant: message.participant || undefined
                },
                message: message.message || this._buildMessageContent(message),
                messageTimestamp: message.timestamp || Math.floor(Date.now() / 1000),
                pushName: message.pushName || message.senderName || 'Unknown'
            };
        } catch (error) {
