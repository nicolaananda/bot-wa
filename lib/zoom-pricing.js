...
function getConfig(tier = 100) {
  ...
  const legacyHour = t === 100 ? readEnvNumber('ZOOM_BUY_PRICE_PER_HOUR', def.perHour) : def.perHour
  const legacyDay = t === 100 ? readEnvNumber('ZOOM_BUY_PRICE_PER_DAY', def.perDay) : def.perDay
  const legacyThr =
    t === 100
...
function describeRates(tier = 100) {
  ...
  return {
    ...
    text:
      ...
      `• 1 minggu (7 hari) = Rp ${fmt(7 * cfg.perDay)}\n` +
      `• 1 bulan (30 hari) = Rp ${fmt(30 * cfg.perDay)}`,
  }
}
