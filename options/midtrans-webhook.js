...
async function ensureWebhookSchema(pg) {
  ...
  await pg.query('ALTER TABLE midtrans_webhooks ADD COLUMN IF NOT EXISTS last_error TEXT');
  await pg.query('ALTER TABLE midtrans_webhooks ADD COLUMN IF NOT EXISTS event_key TEXT');
  await pg.query('CREATE UNIQUE INDEX IF NOT EXISTS uniq_midtrans_webhooks_event_key ON midtrans_webhooks(event_key)');
}
...
function startWebhookWorker(options, intervalMs = 500) {
  ...
  ensureWebhookSchema(options.pg)
    .then(() => { if (!stopped) timer = setTimeout(run, 0); })
    .catch(error => console.error('[Webhook worker] schema migration failed:', error.message));
  return () => { stopped = true; clearTimeout(timer); };
}

module.exports = { createWebhookHandler, ensureWebhookSchema, eventKey, persistWebhook, processNextWebhook, startWebhookWorker };
