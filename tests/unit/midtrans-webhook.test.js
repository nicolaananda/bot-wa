...
const { createWebhookHandler, ensureWebhookSchema, persistWebhook, processNextWebhook } = require('../../options/midtrans-webhook');

function response() { return { code: 0, body: null, status(n) { this.code=n; return this; }, json(v) { this.body=v; return this; } }; }
test('schema migration adds worker columns idempotently', async () => {
  const calls=[]; await ensureWebhookSchema({query:async sql=>{calls.push(sql); return {rows:[]};}});
  assert.equal(calls.length,4); assert.ok(calls.every(sql=>sql.includes('IF NOT EXISTS'))); assert.ok(calls.some(sql=>sql.includes('last_error')));
});
