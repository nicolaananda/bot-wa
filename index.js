...
function cancelAutoDelete(messageKey) {
  const key =
    typeof messageKey === 'string' ? messageKey : messageKey.id || JSON.stringify(messageKey)
  ...
  process.on('payment-completed', async (webhookData) => {
    try {
      ...
      for (const [sender, order] of Object.entries(orders)) {
        ...
        console.log(
          `  - Checking order ${order.orderId || 'N/A'}: Amount Rp${orderAmount}, Diff: Rp${amountDiff.toFixed(2)}`
        )
        ...
        if (amountDiff < 1) {
          ...
          console.log(
            `✅ [MID-GLOBAL] Found matching order: ${order.orderId}, Amount: Rp${orderAmount}, Sender: ${sender}`
          )
          break
        }
      }
      ...
      if (!matchedOrder || !matchedSender) {
        ...
        try {
          ...
          const existingWebhook = db.data.midtransWebhooks.find(
            (w) =>
              w.orderId === webhookOrderId ||
              (Math.abs(Number(w.gross_amount) - webhookAmount) < 1 &&
                w.timestamp > Date.now() - 60000)
          )
      ...
      // IDEMPOTENCY CHECK
      if (order.processed) {
        console.log(`⚠️ [MID-GLOBAL] Order ${orderId} already processed, skipping duplicate event.`)
        return
      }
      ...
      if (currentStock < jumlah) {
        ...
        try {
          ...
          console.error(
            `❌ [MID-GLOBAL] Error sending out-of-stock notification:`,
            notifError.message
          )
        }
...
module.exports = async (nicola, m, mek) => {
  ...
  try {
    ...
    const isOwner = [nicola.user.id, ...owner]
        ...
        while (true) {
          try {
            ...
            if (!transient || attempt > maxRetries) {
              console.warn(
                `WARNING: Failed to fetch group metadata for ${from} after ${attempt} attempts: ${e.message}`
              )
            ...
            console.warn(
              `[GROUP-META] Retry attempt ${attempt}/${maxRetries}, waiting ${backoff}ms...`
            )
            await __delay(backoff)
          }
        }
    ...
    const reply = (teks, options = {}) => {
      nicola.sendMessage(from, { text: teks, ...options }, { quoted: m })
    }
        ...
        from,
        {
          ...
          contextInfo: {
            ...
            externalAdReply: {
              ...
              title: `${botName} © ${ownerName}`,
              body: ownerName + botName,
              thumbnail: getThumbnailBuffer(),
              sourceUrl: linkGroup,
              mediaType: 1,
    ...
    const mentionByTag =
      m.mtype == 'extendedTextMessage' && m.message.extendedTextMessage.contextInfo != null
    ...
    if (!db.data.setting[botNumber])
      db.data.setting[botNumber] = {
        autoread: true,
        autoketik: false,
        anticall: true,
      }
    ...
    function toRupiah(angka) {
      ...
      for (var i = 0; i < angkarev.length; i++) if (i % 3 == 0) saldo += angkarev.substr(i, 3) + '.'
      return (
        '' +
        saldo
      ...
      if (!fromMe) {
        if (db.data.topup[sender].session == 'INPUT-TUJUAN') {
          ...
          axios.get('https://okeconnect.com/harga/json?id=905ccd028329b0a').then(async (res) => {
            ...
            if (
              product.produk == 'TPG Diamond Mobile Legends' ||
              product.produk == 'TPG Genshin Impact Crystals'
            ) {
              if (!chats.split(' ')[1])
                return reply(
            ...
            db.data.topup[sender].data.price = hargaSetelahProfit(
              product.harga,
              db.data.users[sender].role,
              product.kategori
            )
          ...
          if (chats.toLowerCase() == 'lanjut') {
            axios.get('https://okeconnect.com/harga/json?id=905ccd028329b0a').then(async (res) => {
              ...
              if (db.data.users[sender].saldo < db.data.topup[sender].data.price) {
                reply(
                  'Saldo kamu tidak mencukupi untuk melakukan transaksi ini, sesaat lagi bot akan mengirimkan Pembayaran Otomatis.'
                )
                ...
                while (!statusPay) {
                  ...
                  try {
                    let orkut = new OrderKuota(
                      db.data.orkut['username'],
                      db.data.orkut['authToken']
                    )
                    let response = await orkut.getTransactionQris()
                    let result = response.qris_history.results.find(
                      (i) =>
                        i.status == 'IN' && Number(i.kredit.replace(/[.]/g, '')) == parseInt(amount)
                    )
                    ...
                    if (result !== undefined) {
                      ...
                      axios
                        .get(
                          `https://b2b.okeconnect.com/trx-v2?product=${product.kode}&dest=${db.data.topup[sender].data.id}${db.data.topup[sender].data.zone}&refID=${db.data.topup[sender].id}&memberID=${memberId}&pin=${pin}&password=${pw}`
                        )
                        .then(async (ress) => {
                          if (ress.data.status == 'GAGAL') {
                            ...
                            await dbHelper.updateUserSaldo(
                              sender,
                              db.data.topup[sender].data.price,
                              'add'
                            )
                            delete db.data.topup[sender]
                          } else {
                          ...
                          while (status !== 'SUKSES') {
                            ...
                            if (responses.status == 'SUKSES') {
                              if (
                                ...
                                let invoice = await generateInvoiceWithBackground(
                                  data,
                                  './options/image/bg.jpg'
                                )
                                await sleep(200)
                                await nicola.sendMessage(
                                  from,
                                  {
                                    image: fs.readFileSync(invoice),
    ...
    if (command === 'payqris') {
      if (!db.data.deposit[sender]) {
        ...
        reply(
          'Proses deposit kamu masih ada yang belum terselesaikan.\n\nKetik *batal* untuk membatalkan.'
        )
      }
...
Total Bayar: Rp${toRupiah(db.data.deposit[sender].data.total_deposit)}`
                    ...
                    await dbHelper.updateUserSaldo(
                      sender,
                      Number(db.data.deposit[sender].data.amount_deposit),
                      'add'
                    )
                    delete db.data.deposit[sender]
                  }
...
_Silahkan transfer dengan nomor yang sudah tertera, jika sudah harap kirim bukti foto dengan caption *bukti* untuk di acc oleh Admin._`
    ...
    if (isGroup && isAlreadyResponList(from, chats.toLowerCase())) {
      ...
      if (get_data_respon.isImage === false) {
        nicola.sendMessage(
          from,
          { text: sendResponList(from, chats.toLowerCase()) },
          {
            quoted: m,
          }
        )
        ...
        nicola.sendMessage(
          from,
          { image: { url: get_data_respon.image_url }, caption: get_data_respon.response },
          {
            quoted: m,
          }
        )
    ...
    if (isGroup && db.data.chat[from].antilink) {
      ...
      if (chats.match(/(`https:\/\/chat.whatsapp.com\/${gc}`)/gi)) {
        ...
        (chats.match('http://') ||
          ...
          chats.match('wa.me') ||
          chats.match('t.me')) &&
        !chats.match(`https://chat.whatsapp.com/${gc}`)
      ) {
        if (!isBotGroupAdmins) return
        if (!isOwner && !isGroupAdmins) {
          await nicola.sendMessage(from, { delete: m.key })
          nicola.sendMessage(from, {
            text: `*LINK DETECTOR*\n\nMaaf @${sender.split('@')[0]}, sepertinya kamu mengirimkan link, maaf kamu akan di kick.`,
            mentions: [sender],
          })
          await sleep(500)
    ...
    if (isGroup && db.data.chat[from].antilink2) {
      ...
      if (
        ...
        if (!isOwner && !isGroupAdmins) {
          await nicola.sendMessage(from, { delete: m.key })
          nicola.sendMessage(from, {
            text: `*LINK DETECTOR*\n\nMaaf @${sender.split('@')[0]}, sepertinya kamu mengirimkan link, lain kali jangan kirim link yaa.`,
            mentions: [sender],
          })
        }
      }
    ...
    switch (command) {
      ...
      case 'testmsg':
        ...
        try {
          ...
          await nicola.sendMessage(testNumber, {
            text: '🧪 Test 1: Pesan sederhana - apakah sampai?',
          })
...
Jika pesan ini sampai, sistem berfungsi normal.`
      ...
      case 'resendakun':
        ...
        if (!q)
          return reply(
            '❌ Format: .resendakun <nomor_customer> <product_id> <jumlah>\nContoh: .resendakun 6281234567890 vid3u 1'
          )
        ...
        if (db.data.produk[productId].stok.length < qty) {
          return reply(
            `❌ Stok tidak cukup. Stok tersedia: ${db.data.produk[productId].stok.length}`
          )
        }
        ...
        try {
          ...
          reply(
            `✅ Detail akun berhasil dikirim ulang ke ${customerNum}\n\nProduk: ${db.data.produk[productId].name}\nJumlah: ${qty} akun`
          )
        } catch (error) {
      ...
      case 'stock':
        {
          try {
            ...
            // Send the message
            const sentMessage = await nicola.sendMessage(
              from,
              {
                text: teks,
      ...
      case 'addproduk':
        {
          ...
          if (!data[5])
            return reply(
              `Contoh: ${prefix + command} id|namaproduk|deskripsi|snk|harga bronze|harga silver|harga gold|profit`
            )
          if (db.data.produk[data[0]])
            return reply(`Produk dengan ID ${data[0]} sudah ada di database`)
      ...
      case 'setharga':
        {
          ...
          if (!db.data.produk[data[0]])
            return reply(`Produk dengan ID *${data[0]}* tidak ada di database`)

          if (data[1].toLowerCase() == 'bronze') {
            db.data.produk[data[0]].priceB = Number(data[2])
            reply(
              `Berhasil set harga produk dengan ID *${data[0]}* menjadi Rp${toRupiah(Number(data[2]))}`
            )
      ...
      case 'rekap':
        ...
        async function checkPaymentViaPG({ totalAmount, createdAtISO, deviceId = null }) {
          const url =
            `${PG_ENDPOINT}/notifications?limit=50` +
          ...
          const match = items.find((n) => {
            try {
              ...
              const appOk =
                n.packageName === 'com.gojek.gopaymerchant' ||
                String(n.appName || '')
                  .toUpperCase()
                  .includes('GOPAY') ||
                String(n.appName || '')
                  ...
                  .includes('GOJEK')
              const textOk =
                /menerima|received/i.test(String(n.text || '')) ||
                /masuk/i.test(String(n.text || ''))
      ...
      case 'deposit':
        {
          ...
          if (!Number.isFinite(baseAmount) || baseAmount <= 0)
            return reply('Nominal harus berupa angka lebih dari 0')

          if (!db.data.users[sender]) db.data.users[sender] = { saldo: 0, role: 'bronze' }
          if (!db.data.orderDeposit) db.data.orderDeposit = {}
          if (db.data.orderDeposit[sender])
            return reply(
              `Kamu sedang memiliki deposit yang belum selesai. Ketik *${prefix}batal* untuk membatalkan.`
            )
          ...
          try {
            ...
            const paymentListener = async (webhookData) => {
              try {
                ...
                console.log(
                  `🔍 [DEPOSIT-LOCAL] Checking webhook: Amount Rp${webhookAmount}, Order Amount Rp${orderAmount}, Diff: Rp${amountDiff.toFixed(2)}, Match: ${isAmountMatch}`
                )

                // Match jika amount sama (dengan tolerance) dan status paid
                if (isAmountMatch && isStatusPaid) {
                  console.log(
                    `✅ [DEPOSIT] Webhook payment detected: ${orderId}, Amount: ${webhookAmount}`
                  )
                  ...
                  try {
                    ...
                    console.error(
                      '❌ [DEPOSIT] Failed to record saldo history:',
                  ...
                  try {
                    ...
                    console.error(
                      '❌ [DEPOSIT] Failed to record deposit transaction:',
                  ...
                  try {
                    ...
                    console.error(
                      `⚠️ [DEPOSIT] Telegram notification failed:`,
                  ...
                  await db.save()
                  console.log(
                    `✅ [DEPOSIT] Transaction completed via webhook: ${orderId} - ${reffId}`
                  )
                }
              } catch (error) {
            ...
            while (db.data.orderDeposit[sender]) {
              ...
              if (pollCount % 2 === 0) {
                // Cek setiap 2 polling cycle (sekitar 4-8 detik) - lebih cepat untuk deteksi
                try {
                  if (usePg && pg) {
               ...
               LIMIT 10`,
                    ...
                    if (result.rows.length > 0) {
                      console.log(
                        `🔍 [DEPOSIT] Found ${result.rows.length} unprocessed webhooks from PostgreSQL, looking for amount Rp${orderAmount}, created after ${orderCreatedAt.toISOString()}`
                      )
                    }
                    ...
                    for (const row of result.rows) {
                      ...
                      if (isAmountMatch && isTimeValid && isOrderIdValid) {
                        console.log(
                          `✅ [DEPOSIT] Payment detected via PostgreSQL webhook: Amount Rp${webhookAmount}, OrderID: ${row.order_id}`
                        )
                   ...
                   WHERE id = $1`,
                    ...
                    const unprocessedWebhooks = webhooks.filter(
                      (w) => !w.processed && w.gross_amount
                    )

                    if (unprocessedWebhooks.length > 0) {
                      console.log(
                        `🔍 [DEPOSIT] Checking ${unprocessedWebhooks.length} unprocessed webhooks from JSON, looking for amount Rp${totalAmount}, created after ${new Date(createdAtTs).toISOString()}`
                      )
                    }
                    ...
                    for (const webhook of unprocessedWebhooks) {
                      ...
                      const isStatusPaid = /(settlement|capture)/i.test(
                        String(webhook.transactionStatus)
                      )
                      ...
                      if (isAmountMatch && isStatusPaid && isTimeValid && isOrderIdValid) {
                        console.log(
                          `✅ [DEPOSIT] Payment detected via JSON webhook: Amount Rp${webhookAmount}, OrderID: ${webhook.orderId}`
                        )
      ...
      case 'buynow':
        {
          ...
          try {
            ...
            try {
              ...
              console.log(
                `📝 [MID] Order created: ${orderId}, Amount: Rp${totalAmount}, Sender: ${sender}`
              )
              ...
              const paymentListener = async (webhookData) => {
                try {
                  ...
                  console.log(
                    `🔍 [MID-LOCAL] Checking webhook: Amount Rp${webhookAmount}, Order Amount Rp${orderAmount}, Diff: Rp${amountDiff.toFixed(2)}, Match: ${isAmountMatch}`
                  )

                  // Match jika amount sama (dengan tolerance) dan status paid
                  if (isAmountMatch && isStatusPaid) {
                    console.log(`✅ [MID-LOCAL] Webhook match detected for ${orderId}`)

                    // IDEMPOTENCY CHECK:
                    // We rely on the GLOBAL listener (lines 340+) to handle dataset updates, message sending, and logging.
                    // This local listener only ensures the specific QR bubble in this context is deleted
                    // if the global listener hasn't reached it yet or if we want immediate feedback.

                    // However, to prevent race conditions where both try to process:
                    if (order.processed) {
                      console.log(
                        `⚠️ [MID-LOCAL] Order ${orderId} already processed by Global Listener. Removing local listener.`
                      )
                      process.removeListener('payment-completed', paymentListener)
                      return
                    }

                    // If global hasn't picked it up yet (race condition), let's mark it here too?
                    // No, safer to just let Global listener handle the business logic.
                    // We just wait or do nothing.
                    // But user wants immediate feedback.

                    // Decision: We will let the GLOBAL listener do the work.
              ...
              while (db.data.order[sender]) {
                ...
                // Fallback: Cek webhook dari PostgreSQL database (karena dashboard-api dan bot-wa adalah process terpisah)
                if (pollCount % 2 === 0) {
                  // Cek setiap 2 polling cycle (sekitar 6-10 detik) - lebih cepat untuk deteksi
                  try {
                    if (usePg && pg) {
                   ...
                   LIMIT 10`,
                      ...
                      if (result.rows.length > 0) {
                        console.log(
                          `🔍 [MID] Found ${result.rows.length} unprocessed webhooks from PostgreSQL, looking for amount Rp${orderAmount}, created after ${orderCreatedAt.toISOString()}`
                        )
                      }
                      ...
                      for (const row of result.rows) {
                        ...
                        if (isAmountMatch && isTimeValid && isOrderIdValid) {
                          console.log(
                            `✅ [MID] Payment detected via PostgreSQL webhook: Amount Rp${webhookAmount}, OrderID: ${row.order_id}`
                          )
                      ...
                      if (unprocessedWebhooks.length > 0) {
                        console.log(
                          `🔍 [MID] Checking ${unprocessedWebhooks.length} unprocessed webhooks from JSON, looking for amount Rp${totalAmount}, created after ${new Date(createdAtTs).toISOString()}`
                        )
                      }
                      ...
                      for (const webhook of unprocessedWebhooks) {
                        ...
                        if (isAmountMatch && isStatusPaid && isTimeValid && isOrderIdValid) {
                          console.log(
                            `✅ [MID] Payment detected via JSON webhook: Amount Rp${webhookAmount}, OrderID: ${webhook.orderId}`
                          )
                ...
                if (USE_POLLING && pollCount % 5 === 0) {
                  // Cek setiap 5 polling cycle
                  try {
      ...
      case 'buy':
        {
          ...
          try {
            ...
            if (isOwner && data.length >= 3) {
              ...
              return reply(
                `ℹ️ Parameter ketiga diabaikan. Untuk membeli akun dan mengirim ke nomor lain, hubungi owner/admin.\n\nGunakan format: ${prefix + command} ${data[0]} ${data[1]}`
              )
            }
            ...
            try {
              ...
              reply(
                isOwnerBuy
                  ? `Sedang memproses pembelian untuk nomor ${cleanedNumber || targetNumber?.replace('@s.whatsapp.net', '') || 'N/A'}.`
              ...
              if (isOwnerBuy) {
                ...
                console.log(
                  `   - Target User: ${cleanedNumber || targetNumber?.replace('@s.whatsapp.net', '') || 'N/A'}`
                )
              ...
              if (customerMessageSent) {
                if (isOwnerBuy) {
                  ...
                  reply(
                    '🎉 Pembelian dengan saldo berhasil! Detail akun telah dikirim ke chat pribadi Anda. Terima kasih!'
                  )
                } else {
                  reply(
                ...
                if (isOwnerBuy) {
                  reply(
                    `⚠️ Pembelian berhasil, tetapi terjadi masalah saat mengirim detail akun ke nomor ${cleanedNumber || targetNumber?.replace('@s.whatsapp.net', '') || 'N/A'}. Silakan coba kirim ulang atau hubungi admin.`
                  )
                } else {
                  reply(
                    '⚠️ Pembelian dengan saldo berhasil, tetapi terjadi masalah saat mengirim detail akun. Admin akan segera mengirim detail akun secara manual.'
                  )
                }
      ...
      case 'batal':
        {
          ...
          if (m.quoted && (isOwner || isGroupAdmins)) {
            ...
            if (quotedSender && quotedSender !== sender) {
              ...
              if (cancelled) {
                reply(`✅ Berhasil membatalkan pembayaran user @${quotedSender.split('@')[0]}`, {
                  mentions: [quotedSender],
                })
              } else {
                reply(
                  `❌ Tidak ada pembayaran yang sedang berlangsung untuk user @${quotedSender.split('@')[0]}`,
                  { mentions: [quotedSender] }
                )
              }
              break
      ...
      case 'gpt':
        try {
          ...
          const matchedProducts = Object.entries(products).filter(
            ([id, product]) => product.name && product.name.toLowerCase().includes(keyword)
          )
      ...
      case 'isi':
        {
          ...
          if (!isQuotedMsg)
            return reply(
              `Reply pesan orang yang ingin diisi saldonya dengan caption *${prefix + command} nominal*\n\nContoh: ${prefix + command} 100000`
            )
          if (!q)
            return reply(
      ...
      case 'antilink':
        {
          ...
          if (q.toLowerCase() == 'on') {
            ...
            if (!db.data.chat[from].antilink)
              return reply('Antilink sudah tidak aktif di grup ini.')
      ...
      case 'antilinkv2':
        {
          ...
          if (q.toLowerCase() == 'on') {
            ...
            if (!db.data.chat[from].antilink2)
              return reply('Antilinkv2 sudah tidak aktif di grup ini.')
      ...
      case 'sendagain':
        try {
          ...
          if (!receiptResult.success) {
            ...
            console.log(
              `⚠️ [RESEND] Receipt not found for ${lastTransaksi.reffId} (checked R2 and local)`
            )
          ...
          console.log(
            `✅ [RESEND] Receipt found from ${receiptResult.storage || 'storage'} for ${lastTransaksi.reffId}`
          )
          ...
          console.log(
            `🔁 [RESEND] User ${userPhone} requested resend for transaction ${lastTransaksi.reffId}`
          )
        } catch (err) {
      ...
      case 'saldotoday':
        {
          try {
            ...
            if (transaksiSaldo.length === 0) {
              return reply(
                `📊 Tidak ada transaksi saldo pada ${moment.tz('Asia/Jakarta').format('DD MMMM YYYY')}`
              )
            }
      ...
      case 'done':
        {
          ...
          if (q.startsWith('@')) {
            if (db.data.chat[from].sDone.length !== 0) {
              ...
              nicola.sendMessage(from, {
                text: textDone
                  .replace('tag', q.replace(/[^0-9]/g, ''))
                  .replace('@jam', jamwib)
                  .replace('@tanggal', moment.tz('Asia/Jakarta').format('DD MMMM YYYY'))
                  .replace('@status', 'Berhasil'),
                mentions: [q.replace(/[^0-9]/g, '') + '@s.whatsapp.net'],
              })
            } else {
              nicola.sendMessage(
                from,
                {
                  text: `「 *TRANSAKSI BERHASIL* 」\n\n\`\`\`📆 TANGGAL : ${moment.tz('Asia/Jakarta').format('DD MMMM YYYY')}\n⌚ JAM : ${jamwib}\n✨ STATUS: Berhasil\`\`\`\n\nTerimakasih @${q.replace(/[^0-9]/g, '')} next order yaa🙏`,
                  mentions: [q.replace(/[^0-9]/g, '') + '@s.whatsapp.net'],
                },
                { quoted: m }
              )
            ...
            if (db.data.chat[from].sDone.length !== 0) {
              ...
              nicola.sendMessage(
                ...
                {
                  text: textDone
                    .replace('tag', m.quoted.sender.split('@')[0])
                    .replace('@jam', jamwib)
                    .replace('@tanggal', moment.tz('Asia/Jakarta').format('DD MMMM YYYY'))
                    .replace('@status', 'Berhasil'),
              ...
              nicola.sendMessage(from, {
                text: `「 *TRANSAKSI BERHASIL* 」\n\n\`\`\`📆 TANGGAL : ${moment.tz('Asia/Jakarta').format('DD MMMM YYYY')}\n⌚ JAM : ${jamwib}\n✨ STATUS: Berhasil\`\`\`\n\nTerimakasih @${m.quoted.sender.split('@')[0]} next order yaa🙏`,
                mentions: [m.quoted.sender],
              })
            }
          } else {
      ...
      case 'helpadmin': {
        ...
        const text =
          ...
          `• \`${p}zoomlicense<tier>\` — audit lisensi semua host\n` +
          `• \`${p}zoomlicense<tier> refresh\` — force re-fetch dari Zoom\n` +
          ...
          `• \`${p}delproduk <id>\` — hapus produk\n` +
          `• \`${p}setharga <id> <bronze> <silver> <gold>\` — set harga\n` +
          `• \`${p}setjudul <id> <judul>\` — ubah nama produk\n` +
          `• \`${p}setdesk <id> <desc>\` — ubah deskripsi\n` +
          ...
          `• \`${p}isi @user <nominal>\` — set saldo absolut\n` +
          ...
          `• \`${p}anticall on|off\` — auto-block caller\n` +
          `• \`${p}kick @user\` — kick dari group\n` +
          `• \`${p}blok\` / \`${p}block\` — block contact\n` +
          `• \`${p}unblok\` / \`${p}unblock\` — unblock\n` +
          ...
          `• \`${p}done\` — selesaikan transaksi pending\n` +
