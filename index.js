...
*╰┈┈┈┈┈┈┈┈*`, mentions: [sender] })
          ...
          // Simpan detail transaksi lengkap (termasuk akun yang dikirim) ke options/database-transaksi.json
          try {
            const transaksiFilePath = './options/database-transaksi.json'
            let transaksiDetailDB = []
            if (fs.existsSync(transaksiFilePath)) {
              try {
                transaksiDetailDB = JSON.parse(fs.readFileSync(transaksiFilePath, 'utf8'))
                if (!Array.isArray(transaksiDetailDB)) transaksiDetailDB = []
              } catch (e) {
                transaksiDetailDB = []
              }
            }

            const deliveredAccounts = dataStok.map((i) => {
            ...
            const record = {
              ...
              idProduk: data[0],
              produk: db.data.produk[data[0]].name,
              hargaSatuan: Number(hargaProduk(data[0], db.data.users[sender].role)),
              jumlah: jumlah,
              totalBayar: totalHarga,
              metodeBayar: 'Saldo',
            ...
            // Upsert by reffId
            const existingIndex = transaksiDetailDB.findIndex(r => r && r.reffId === reffId)
            if (existingIndex >= 0) transaksiDetailDB[existingIndex] = record
            else transaksiDetailDB.push(record)

            fs.writeFileSync(transaksiFilePath, JSON.stringify(transaksiDetailDB, null, 2), 'utf8')
          } catch (e) {
            console.log('Gagal menyimpan database-transaksi.json:', e)
          }
